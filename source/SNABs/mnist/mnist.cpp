...
cypress::Network &SimpleMnist::build_netw(cypress::Network &netw)
{
	...
	/*size_t image = 0;
	std::cout << std::get<1>(data)[image] << ", " << image << std::endl;
	mnist_helper::print_image(std::get<0>(data)[image], 28);
	image = 3;
	std::cout << std::get<1>(data)[image] << ", " << image << std::endl;
	mnist_helper::print_image(std::get<0>(data)[image], 28);
	image = 5;
	...
	/*Utilities::write_vector2_to_csv(std::get<0>(batch_data[0]), "spikes.csv");
	Utilities::plot_spikes("spikes.csv", m_backend);*/
...
std::vector<std::array<cypress::Real, 4>> SimpleMnist::evaluate()
{
	...
	auto labels = mnist_helper::spikes_to_labels(spikes, m_duration, m_pause, m_batchsize);
    auto& orig_labels = std::get<1>(m_batch_data[0]);
    auto correct = mnist_helper::compare_labels(orig_labels, labels);
    
    std::cout << "\n__________\nFinally Correct: " << correct <<std::endl;
...
#endif
	return {std::array<cypress::Real, 4>({Real(correct), NaN(), NaN(),NaN()})};
}
...
void SimpleMnist::create_deep_network(const Json &data, Network &netw)
{
	...
	for (auto &layer : data["netw"]) {
		if (layer["class_name"].get<std::string>() == "Dense") {
			...
			auto conns =
			    mnist_helper::dense_weights_to_conn(layer["weights"], 
                                                    m_max_weight/max, 1.0);
